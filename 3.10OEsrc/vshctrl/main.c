...
void IoPatches()
{
	...
	{	
		...
		PatchSyscall(text_addr+0x2C24, sceIoOpenPatched);
		PatchSyscall(text_addr+0x2BE4, sceIoClosePatched);
		PatchSyscall(text_addr+0x2D3C, sceIoReadPatched);
		PatchSyscall(text_addr+0x2DAC, sceIoLseekPatched);
		PatchSyscall(text_addr+0x2DE4, sceIoLseek32Patched);
		PatchSyscall(text_addr+0x2ED8, sceIoGetstatPatched);
		PatchSyscall(text_addr+0x2EF8, sceIoChstatPatched);
		PatchSyscall(text_addr+0x07F8, sceIoRemovePatched);
		PatchSyscall(text_addr+0x2E98, sceIoRmdirPatched);
		PatchSyscall(text_addr+0x2E7C, sceIoMkdirPatched);
...
void PatchVshMain(u8 *buf)
{
	...
	// Allow old sfo's.
	_sw(NOP, (u32)(text_addr+0xE968));
	_sw(NOP, (u32)(text_addr+0xE970)); 
...
void PatchSysconfPlugin(u8 *buf)
{
	...
	memcpy((void *)(text_addr+0x1AD7C), verinfo, sizeof(verinfo));
	
	addrhigh = (text_addr+0x1AD7C-0xA0) >> 16;
	addrlow = (text_addr+0x1AD7C-0xA0) & 0xFFFF;

	// lui v0, addrhigh
	_sw(0x3c020000 | addrhigh, text_addr+0xE11C);
	// ori v0, v0, addrlow
	_sw(0x34420000 | addrlow, text_addr+0xE120);
...
void PatchMsVideoMainPlugin(u8 *buf)
{
	...
	_sh(0x4003, text_addr+0x35F00);
	_sh(0x4003, text_addr+0x35F8C);
	_sh(0x4003, text_addr+0x36028);
...
{
	u32 text_addr = (u32)(buf+0xA0);
	_sw(0x1000fff9, text_addr+0xC4E0);
	_sw(0x24040000, text_addr+0xC4E4);
	ClearCaches();			
}
