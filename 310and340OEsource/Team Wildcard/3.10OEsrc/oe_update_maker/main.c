...
int extract_psar(u8 *dataPSAR, int cbFile, u8 *dataOut, u8 *dataOut2, int is150)
{
                ...
                if (ret == cbExpanded)
                {
					...
					else 
					{
						...
						if (addit)
						{
							...
							{
								sigcheck = 0;
								cbExpanded = sizeof(pspbtcnf310);
								memcpy(dataOut2, pspbtcnf310, cbExpanded);
							}

							else if (strcmp(name, "flash0:/kn/pspbtcnf_game.txt") == 0)
							{
								sigcheck = 0;
								cbExpanded = sizeof(pspbtcnf_game310);
								memcpy(dataOut2, pspbtcnf_game310, cbExpanded);
							}

							else if (strcmp(name, "flash0:/kn/pspbtcnf_updater.txt") == 0)
							{
								sigcheck = 0;
								cbExpanded = sizeof(pspbtcnf_updater310);
								memcpy(dataOut2, pspbtcnf_updater310, cbExpanded);
							}

							else if (strcmp(name, "flash0:/kn/pspbtcnf_pops.txt") == 0)
							{
								sigcheck = 0;
								cbExpanded = sizeof(pspbtcnf_pops310);
								memcpy(dataOut2, pspbtcnf_pops310, cbExpanded);
							}
	...
	else
	{
		...
		WriteFile("ms0:/reboot310.def", dataOut, compsize);
		...
		if (dxarAddFile("flash0:/kn/systemctrl.prx", systemctrl310, sizeof(systemctrl310), 0, 0, dataOut2, 2000000) < 0)
		{
			ErrorExit(6000, "Error (dxar).\n");
		}
...
u8 sha1_310[20] = 
{
	0xE1, 0x34, 0x54, 0x9A, 0x40, 0xD5, 0x05, 0x44, 
	0xAA, 0xA5, 0x59, 0xB7, 0xDD, 0x4B, 0x0E, 0x8D, 
	0xA9, 0xFC, 0x27, 0x2C
};

u8 sha1_dxar[20] = 
{
	0x68, 0x3A, 0xD0, 0xAB, 0xA4, 0x8A, 0x79, 0x68, 
	0xF3, 0x17, 0x2A, 0x5A, 0xB2, 0xE4, 0x1C, 0xF9, 
	0x5A, 0x47, 0x68, 0x01
};
...
int main(int argc, char *argv[])
{
	...
	printf("3.10 OE-A DXAR Update maker by Dark_AleX.\n");
	...
	printf("Opening and checking 310.PBP...\n");

	fd = sceIoOpen("310.PBP", PSP_O_RDONLY, 0777);
	if (fd < 0)
	{
		ErrorExit(6000, "Cannot open file.\n");
	}
	...
	if (size != 16703136)
	{
		ErrorExit(6000, "Invalid 3.10 update file.\n");
		sceIoClose(fd);
	}

	if (sceIoRead(fd, dataPSAR, 16703136) != 16703136)
	{
		ErrorExit(6000, "Invalid 3.10 update file (2).\n");
		sceIoClose(fd);
	}

	sceIoClose(fd);

	sceKernelUtilsSha1Digest(dataPSAR, 16703136, sha1);

	if (memcmp(sha1, sha1_310, 20) != 0)
	{
		ErrorExit(6000, "Invalid 3.10 update file (3).\n");		
	}

	if (dxarInitSection("3.10") < 0)
		ErrorExit(5000, "Error (dxar).\n");

	printf("Extracting and packing 3.10 firmware...\n");	

	extract_psar(dataPSAR, 16703136, dataOut, dataOut2, 0);
