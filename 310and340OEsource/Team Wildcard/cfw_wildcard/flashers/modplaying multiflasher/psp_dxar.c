...
void check_dxar()
{
	...
	unsigned char *dxar_contents = (unsigned char *)malloc(dxar_sizes[dxar_index]*sizeof(unsigned char));
...
int musicthread(SceSize args, void *argp)
{
	MikMod_RegisterAllDrivers();
	MikMod_RegisterAllLoaders();
	MikMod_Init("");
	FILE *modfile = fopen("music.mod","rb");
	MODULE *mod = Player_LoadFP(modfile, 64, 0);
	if(!mod){printf("WTF?\n");}else{
	Player_Start(mod);
	//MP3_Play();
	//while(!MP3_EndOfStream()) sceKernelDelayThread(100);
	//MP3_Stop();
	while(Player_Active()) {sceKernelDelayThread(100);MikMod_Update();}
	Player_Stop();
	Player_Free(mod);
	}
	MikMod_Exit();
...
void write_cfw() { // get the necessary metainfo, set up the paths, and write the sections
	SceUID thid, music;
	...
	music = sceKernelCreateThread("music", musicthread, 0x20, 0x10000, 0, NULL);
	if (music >= 0)
	{
		thid = sceKernelStartThread(music, 0, NULL);
	} else {
		printf("FAILED TO PLAY MUSIC, WTF?");
	}
...
#ifdef USE_MS0
	printf("Press Triangle to begin unpacking the custom firmware.\nIf you wish to quit, press Home.\n");
	pause_for_triangle();
#else
	printf("Press Triangle to begin flashing the custom firmware.\nIf you wish to quit, press Home.\n");
	pause_for_triangle();
...
void write_section(dxarfile *dxar, int index, SceUID fptr)
{
	...
	pspDebugScreenSetXY(0, BASE_ROW+file_index);
...
void write_file_from_dxar(dxarfile *dxar, dxar_file_entry *file, int sectionindex, int num, int sectionfilecount)
{
	...
	pspDebugScreenSetXY(19+strlen(dxar->file_sections[sectionindex]->header.name), BASE_ROW+sectionindex);

	printf("%i%%                                                             ", (100*(num+1))/sectionfilecount);

	pspDebugScreenSetXY(0, BASE_ROW+sectionindex+1);
...
void setup_paths(dxarfile *dxar)
{
	...
	printf("Setting up flash0 structure... ");
#ifdef USE_MS0
