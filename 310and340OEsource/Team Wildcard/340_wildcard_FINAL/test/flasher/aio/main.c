...
#include "../common/dxar_info.h"
#include "dxar.h"
#include "idcanager.h"
#include "multi_cfw_installer.h"
#include "popcorn.h"
#include "pspbtcnf340.h"
#include "pspbtcnf_game150.h"
#include "pspbtcnf_game340.h"
#include "pspbtcnf_licensegame340.h"
#include "pspbtcnf_pops340.h"
#include "pspbtcnf_updater150.h"
#include "pspbtcnf_updater340.h"
#include "reboot150.h"
#include "recovery.h"
#include "systemctrl150.h"
#include "systemctrl340.h"
#include "uart4.h"
#include "vshctrl.h"
...
PSP_MODULE_INFO("se_update_maker", 0x1000, 1, 0);
...
int extract_psar(u8 *dataPSAR, int cbFile, u8 *dataOut, u8 *dataOut2, int is150)
{
                ...
                if (ret == cbExpanded)
                {
                    if (is150)
					{
						for (i = 0; i < 112; i++)
						{
							if (strcmp(name, subset150[i]) == 0)
							{
								...
								if (dxarAddFile(name, dataOut2, cbExpanded, 1, 0, dataOut, 2000000) < 0)
								{
					...
					else 
					{
						...
						if (addit)
						{
							...
							if (strcmp(name, "flash0:/kn/pspbtcnf.txt") == 0)
							{
								sigcheck = 0;
								cbExpanded = sizeof(pspbtcnf340);
								memcpy(dataOut2, pspbtcnf340, cbExpanded);
							}

							else if (strcmp(name, "flash0:/kn/pspbtcnf_game.txt") == 0)
							{
								sigcheck = 0;
								cbExpanded = sizeof(pspbtcnf_game340);
								memcpy(dataOut2, pspbtcnf_game340, cbExpanded);
							}

							else if (strcmp(name, "flash0:/kn/pspbtcnf_updater.txt") == 0)
							{
								sigcheck = 0;
								cbExpanded = sizeof(pspbtcnf_updater340);
								memcpy(dataOut2, pspbtcnf_updater340, cbExpanded);
							}

							else if (strcmp(name, "flash0:/kn/pspbtcnf_pops.txt") == 0)
							{
								sigcheck = 0;
								cbExpanded = sizeof(pspbtcnf_pops340);
								memcpy(dataOut2, pspbtcnf_pops340, cbExpanded);
							}
							else if (strcmp(name, "flash0:/kn/pspbtcnf_licensegame.txt") == 0)
							{
								sigcheck = 0;
								cbExpanded = sizeof(pspbtcnf_licensegame340);
								memcpy(dataOut2, pspbtcnf_licensegame340, cbExpanded);
							}

							if (strncmp(name, "ipl", 3) != 0)
							{
								if (dxarAddFile(name, dataOut2, cbExpanded, 1, sigcheck, dataOut, 2000000) < 0)
								{
									ErrorExit(6000, "Error (dxar) %s.\n", name);
								}
								...
								{
									if (dxarAddFile("flash0:/font/kr0.pgf", dataOut2, cbExpanded, 1, sigcheck, dataOut, 2000000) < 0)
									{
										ErrorExit(6000, "Error (dxar) %s.\n", name);
									}
								}
							...
							else if (strcmp(name, "flash0:/kn/sysmem.prx") == 0)
							{
								...
								for (i = 0; i < sz-22; i++)
								{
									if (memcmp(dataOut+i, "sceSystemMemoryManager", 22) == 0)
									{
										memcpy(dataOut+i, "dax", 3);
										break;
									}
								}

								pspSdkInstallNoPlainModuleCheckPatch();
								SceUID mod = sceKernelLoadModuleBuffer(dataOut, sz, 0, NULL);

								if (mod < 0)
								{
									ErrorExit(6000, "Cannot load 2.71 sysmem.prx.\n");
	...
	if (is150)
	{
		for (i = 0; i < sizeof(reboot150); i++)
		{
			if (memcmp(reboot150+i, "reboot.bin", 10) == 0)
			{
				...
				if (dxarAddFile("flash0:/kn/reboot150.prx", reboot150, sizeof(reboot150), 1, 0, dataOut2, 2000000) < 0)
				{
		...
		if (dxarAddFile("flash0:/kd/uart4.prx", uart4, sizeof(uart4), 1, 0, dataOut2, 2000000) < 0)
		{
			ErrorExit(6000, "Error (dxar).\n");
		}

		if (dxarAddFile("flash0:/kd/recovery.prx", recovery, sizeof(recovery), 1, 0, dataOut2, 2000000) < 0)
		{
			ErrorExit(6000, "Error (dxar).\n");
		}
	}
	...
	else
	{
		...
		WriteFile("ms0:/reboot340.def", dataOut, compsize);
		...
		for (i = 0; i < sizeof(systemctrl150); i++)
		{
			if (memcmp(systemctrl150+i, "reboot.bin", 10) == 0)
			{
				...
				if (dxarAddFile("flash0:/kd/rtc.prx", systemctrl150, sizeof(systemctrl150), 1, 0, dataOut2, 2000000) < 0)
				{
					ErrorExit(6000, "Error (dxar).\n");
				}
		...
		if (dxarAddFile("flash0:/kn/idcanager.prx", idcanager, sizeof(idcanager), 1, 0, dataOut2, 2000000) < 0)
		{
			ErrorExit(6000, "Error (dxar).\n");
		}

		if (dxarAddFile("flash0:/kn/popcorn.prx", popcorn, sizeof(popcorn), 1, 0, dataOut2, 2000000) < 0)
		{
			ErrorExit(6000, "Error (dxar).\n");
		}

		if (dxarAddFile("flash0:/kn/systemctrl.prx", systemctrl340, sizeof(systemctrl340), 1, 0, dataOut2, 2000000) < 0)
		{
			ErrorExit(6000, "Error (dxar).\n");
		}

		if (dxarAddFile("flash0:/kn/vshctrl.prx", vshctrl, sizeof(vshctrl), 1, 0, dataOut2, 2000000) < 0)
		{
			ErrorExit(6000, "Error (dxar).\n");
		}
	}
...
u8 sha1_340[20] =
{
        0x3b, 0xae, 0x9c, 0xf5, 0xdc, 0xfe, 0x0d, 0x7e,
        0x3d, 0x7d, 0x5c, 0x6a, 0x23, 0x75, 0x93, 0xaf,
        0x21, 0x95, 0x5f, 0x6d
};
...
int main(int argc, char *argv[])
{
	...
	printf("3.40* DXAR Update maker.\n");
	printf("Based on the work of Dark_AleX.\n");
	printf("Uses code of psardumper (PspPet) for psar extraction.\n");

	if(!alloc) dataPSAR = (u8 *)memalign(0x40, 0x0100a830+256);

	if (!dataPSAR)
		ErrorExit(6000, "Cannot allocate memory (1).\n");

	if(!alloc) dataOut = (u8 *)memalign(0x40, 2000000);

	if (!dataOut)
		ErrorExit(6000, "Cannot allocate memory (2).\n");

	if(!alloc) dataOut2 = (u8 *)memalign(0x40, 2000000);
	...
	if((fd = sceIoOpen("ms0:/DATA.DXAR", PSP_O_RDONLY, 0777)) > 0)
	{
		sceIoClose(fd);
		check_sha1(0);
		if(sha1_ok)
...
void write_dxar()
{
	...
	SceUID fd = sceIoOpen("150.PBP", PSP_O_RDONLY, 0777);
	if (fd < 0)
	{
	...
	printf("Opening and checking 340.PBP...\n");

	fd = sceIoOpen("340.PBP", PSP_O_RDONLY, 0777);
	if (fd < 0)
	{
		ErrorExit(6000, "Cannot open file.\n");
	}
	...
	if (size != 0x0100a830)
	{
		ErrorExit(6000, "Invalid 3.40 update file.\n");
		sceIoClose(fd);
	}

	if (sceIoRead(fd, dataPSAR, 0x0100a830) != 0x0100a830)
	{
		ErrorExit(6000, "Invalid 3.40 update file (2).\n");
		sceIoClose(fd);
	}

	sceIoClose(fd);

	sceKernelUtilsSha1Digest(dataPSAR, 0x0100a830, sha1);

	if (memcmp(sha1, sha1_340, 20) != 0)
	{
		ErrorExit(6000, "Invalid 3.40 update file (3).\n");
	}

	if (dxarInitSection("3.40") < 0)
		ErrorExit(5000, "Error (dxar).\n");

	printf("Extracting and packing 3.40 firmware...\n");

	extract_psar(dataPSAR, 0x0100a830, dataOut, dataOut2, 0);
	free(dataOut2);
...
void check_sha1(int exit)
{
	sha1_ok=1;
	if(!exit)
		printf("Existing DATA.DXAR found.\n");
	printf("Doing static SHA-1 verification...\n");

	SceUID fd = sceIoOpen("ms0:/DATA.DXAR", PSP_O_RDONLY, 0777);
	int i, read;
	...
	if (memcmp(sha1, sha1_340wc, 20) != 0)
	{
		sha1_ok=0;
		...
		if(exit)
		{
			...
			printf("\nERROR: Invalid SHA-1. Rebuilding DXAR.\n");
		}
	}
}
...
void launch_flasher()
{
	...
	SceUID fd = sceIoOpen("ms0:/flasher.elf", PSP_O_WRONLY | PSP_O_CREAT, 0777);
	if(fd < 0)
	{
		pspDebugScreenSetTextColor(RED);
		ErrorExit(5000, "\nCannot open flasher EBOOT for writing.\n");
	}
	
	int written = sceIoWrite(fd, multi_cfw_installer, size_multi_cfw_installer);
	if(written != size_multi_cfw_installer)
	{
		sceIoRemove("ms0:/flasher.elf");
		sceIoClose(fd);
		ErrorExit(5000, "\nCannot write flasher EBOOT.\n");
	}

	sceIoClose(fd);
	printf("\nCorrect value.\nThe dxar creation was succesful.\nLoading flasher in 5 seconds.\n");

	sceKernelDelayThread(5*1000*1000);
	sceKernelLoadExec("ms0:/flasher.elf", NULL);
}
