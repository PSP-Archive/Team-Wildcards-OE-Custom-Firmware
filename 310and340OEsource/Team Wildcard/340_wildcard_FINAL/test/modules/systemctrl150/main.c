...
//#define REBOOT_SIZE (0xB0C8+0x10) // 45,256 + 16 Bytes
//#define REBOOTEX_SIZE (0xA38+0x10) // 2616 + 16Bytes
...
int sceKernelGzipDecompress_fake(u8 *dst, u32 dstSize, const void *src, u32 crc32)
{
	...
	if (isReboot)
	{
		...
		return(sceKernelDeflateDecompress((void *)0x88600000, dstSize, reboot+0x10, crc32));
	}
	else
	{
...
int _start(SceSize args, void *argp)
{
	...
	if (sceKernelFindModuleByName("lcpatcher"))
	{
		...
		// This instruction will be replaced by a branch instruction
		// to skip the following sceKernelExitVSHVSH function to
		// enable the autoboot program to run.
		// This only occurs when autoboot is enabled in recovery.
		asm("nop \n"); 
		asm("nop \n");
