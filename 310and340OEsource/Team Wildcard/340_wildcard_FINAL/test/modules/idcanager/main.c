...
PSP_MODULE_INFO("daxIdCanager_Driver", 0x5006, 1, 0);
PSP_MAIN_THREAD_ATTR(0);
int (* popsman_setkeys)(int dummy, u8 *) = NULL;
int sceIdMgr_driver_F464F91C(int (* func)(int dummy, u8 *))
{
...
int GetOriginalKeys(char *filename, void *keys, char *keyfile)
{
	...
	scePspNpDrm_driver_5667B7B9_2 = (void *)sctrlHENFindFunction("scePspNpDrm_Driver", "scePspNpDrm_driver", 0x5667B7B9);
	...
	if (popsman_setkeys)
		popsman_setkeys(0x10, data+0x1350);
...
SET_KEYS:
	if (popsman_setkeys)
		res = popsman_setkeys(0x10, thekeys);
	else
		res = 0;
	if (res < 0)
	{
...
{
//	WriteFile("ms0:/working.txt", "YES, IT REALLY WORKS", 20, 0777);
	int (* SysMemForKernel_419DB8F4)(void *) = (void *)sctrlHENFindFunction("sceSystemMemoryManager", "SysMemForKernel", 0x419DB8F4);
